...
func (tab table) Crack(hash string) (clair string, err error) {
	...
	for t := tab.Largeur - 1; t > 0; t-- {
		...
		for i := a; i <= b; i++ {
			clair, ok := tab.verifie_candidat(hash, t, tab.Data[i][0])
			if ok {
				return clair, nil
			} else {
				nb_candidats++
...
func (tab table) verifie_candidat(hash string, t uint64, candidat uint64) (string, bool) {
	for i := uint64(1); i < t; i++ {
		candidat = tab.i2i(candidat, i)
	}
	clair := tab.i2c(candidat)
	// check the bytes, not the string. case insensitive
	hb, err := hex.DecodeString(hash)
	if err != nil {
		panic(err) // too bad if it happens, I should check it before
	}
	return clair, bytes.Equal(hb, tab.h(clair))
}
...
func (t table) h2i(hash string, column uint64) uint64 {
